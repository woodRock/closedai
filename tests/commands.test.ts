...
describe('Commands Unit Tests', () => {
  ...
  it('should handle /gitlog', async () => {
    ...
    expect(safeSendMessage).toHaveBeenCalledWith(
      chatId,
      expect.stringContaining('abc - commit message'),
    )
  })

  it('should handle /diff', async () => {
    ;(execSync as any)
      .mockReturnValueOnce(Buffer.from('file.ts | 2 +-'))
  ...
  it('should handle /queue when not empty', async () => {
    const mockDocs = [{ data: () => ({ userMessage: 'test message', attempts: 1 }) }]
    ...
    expect(safeSendMessage).toHaveBeenCalledWith(
      chatId,
      expect.stringContaining('Current Queue (1)'),
    )
  })
  ...
  it('should handle /gitlog error', async () => {
    ...
    expect(safeSendMessage).toHaveBeenCalledWith(
      chatId,
      expect.stringContaining('❌ Failed to fetch git log'),
    )
  })
  ...
  it('should handle /diff error', async () => {
    ...
    expect(safeSendMessage).toHaveBeenCalledWith(
      chatId,
      expect.stringContaining('❌ Failed to fetch diff'),
    )
  })
  ...
  it('should handle /git error', async () => {
    ...
    expect(safeSendMessage).toHaveBeenCalledWith(
      chatId,
      expect.stringContaining('❌ Failed to fetch git info'),
    )
  })
})
