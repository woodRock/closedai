...
describe('Tools Unit Tests', () => {
  ...
  it('search_repo should call execSync with grep', async () => {
    ...
    expect(execSync).toHaveBeenCalledWith(
      'grep -r  --exclude-dir=.git --exclude-dir=node_modules --exclude-dir=dist "search-term" .',
      { cwd: repoRoot },
    )
  ...
  it('git_push with run_tests should run npm test first', async () => {
    const result = await executeTool(
      'git_push',
      { remote: 'origin', branch: 'main', run_tests: true },
      repoRoot,
      chatId,
      mockSafeSendMessage,
    )
    ...
    expect(result).toEqual({
      result: '✅ Pre-flight tests passed.\nSuccess: Pushed to origin main',
    })
  })
  ...
  describe('DX Tools', () => {
    ...
    it('run_tests should run specific file and pattern', async () => {
      ...
      const result = await executeTool(
        'run_tests',
        { path: 'tests/tools.test.ts', pattern: 'format_file' },
        repoRoot,
        chatId,
        mockSafeSendMessage,
      )
      expect(execSync).toHaveBeenCalledWith(
        'npm test -- tests/tools.test.ts -t "format_file"',
        expect.any(Object),
      )
  ...
  describe('Integration & Database Tools', () => {
    ...
    it('db_get_collection_schema should return inferred schema', async () => {
      const { db } = await import('../src/services/firebase.js')
      const mockSnapshot = [{ data: () => ({ name: 'John', age: 30 }) }]
    ...
    it('db_query_collection should return documents', async () => {
      const { db } = await import('../src/services/firebase.js')
      const mockSnapshot = [{ id: '1', data: () => ({ text: 'hello' }) }]
      ;(db.collection as any).mockReturnValue({
        where: vi.fn().mockReturnThis(),
        limit: vi.fn().mockReturnThis(),
        get: vi.fn().mockResolvedValue(mockSnapshot),
