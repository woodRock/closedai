...
async function getChatHistory(chatId: number, limit = 20) {
  ...
  if (summaryDoc.exists) {
    ...
    history.unshift({
      role: 'model',
      parts: [
        { text: 'Understood. I will use this summary as context for our continued interaction.' },
      ],
    })
  }
...
export async function processOneMessage(
  ...
  const allowedUsers = (process.env.ALLOWED_TELEGRAM_USER_IDS || '')
    .split(',')
    .map((s) => s.trim())
  ...
  logInstruction(
    chatId,
    'GEMINI',
    `Processing: ${userMessage ? userMessage.substring(0, 50) : media ? media.mimeType : 'Unknown'}${userMessage && userMessage.length > 50 ? '...' : ''}`,
  )
  ...
  const fileStructure = execSync('find . -maxdepth 2 -not -path "*/.*"', {
    cwd: repoRoot,
  }).toString()
  ...
  try {
      ...
      for (const call of functionCalls) {
        ...
        if (normalizedName === 'run_shell') {
          ...
          const formatted = lines
            .map((l: string, i: number) => `${(i + 1).toString().padStart(3)} | ${l}`)
            .join('\n')
    ...
    if (isGitRepo) {
      try {
        ...
        if (status.length > 0) {
          ...
          if (statusMsg) {
            await bot.telegram
              .editMessageText(chatId, statusMsg.message_id, undefined, finalMsg, {
                parse_mode: 'Markdown',
              })
              .catch(() => safeSendMessage(chatId, finalMsg))
          } else {
    ...
    if (isOverloaded) {
      if (!messageId) {
        ...
        await safeSendMessage(
          chatId,
          '⏳ Gemini is overloaded or unavailable. Your request has been queued and will be retried automatically.',
        )
      } else {
